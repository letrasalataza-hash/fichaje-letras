...
function buildPDFDocument({ title, employeeName, periodLabel, summaryCards, tableHeaders, tableRows, warnings, adjustmentRows = [], signature }) {
  ...
  y = addPDFText(
    doc,
    "El presente informe recoge el registro díario de jornada conforme al artículo 34.9 del Estatuto de los Trabajadores, introducido por el Real Decreto-ley 8/2019. La empresa conservará estos registros durante el periodo legalmente exigible, quedando a disposición de la persona trabajadora, sus representantes legales y la Inspección de Trabajo y Seguridad Social.",
    15,
    y,
  ...
  y = addPDFText(
    doc,
    "Los fichajes originales no deben modificarse ni eliminarse; cualquier corrección deberá constar como ajuste manual trazable. El código de verificación y la huella SHA-256 identifican la version concreta del informe emitido.",
    15,
    y + 2,
    { maxWidth: 180, lineHeight: 4 }
  );
...
function buildDailyReportHTML({ employeeName, filterDate, dailySummary, filteredRecords, signature }) {
  return buildReportHTML({
    title: "Informe díario de fichaje",
    documentLabel: "Informe díario de fichaje",
    employeeName,
...
function buildReportHTML({ title, documentLabel, employeeName, periodLabel, summaryCards, tableHeaders, tableRows, warnings, adjustmentRows = [], signature }) {
  ...
  const legalBlock = `
      <section class="legal-note">
        ...
        <p>El presente informe recoge el registro díario de jornada de la persona trabajadora conforme a lo establecido en el artículo 34.9 del Estatuto de los Trabajadores, introducido por el Real Decreto-ley 8/2019.</p>
        ...
        <p>El código de verificación y la huella SHA-256 identifican la version concreta del informe emitido. Cualquier alteración posterior de los datos utilizados para generar este informe producirá una huella distinta.</p>
      </section>`;
...
function runSelfTests() {
  ...
  ])[0].durationMinutes === 300, "Test duracion timeline");
  ...
  console.assert(summary.adjustmentCount === 1, "Test conteo ajuste díario");
  ...
  console.assert(reportHTML.includes("Informe díario de fichaje"), "Test informe HTML título");
...
function EmployeePortalPanel() {
  ...
  const [isLoadingPortalRecords, setIsLoadingPortalRecords] = useState(false);
  const [portalMonthSummary, setPortalMonthSummary] = useState(null);
  const [isLoadingPortalMonthSummary, setIsLoadingPortalMonthSummary] = useState(false);
  const [portalDocuments, setPortalDocuments] = useState([]);
  const [isLoadingPortalDocuments, setIsLoadingPortalDocuments] = useState(false);
  ...
  function mapPortalRecord(record) {
    const recordedAt = new Date(record.recorded_at);
    return {
      id: record.id,
      type: record.record_type || "registro",
      date: record.local_date || record.recorded_at?.slice(0, 10) || "",
      time: Number.isNaN(recordedAt.getTime()) ? "" : recordedAt.toLocaleTimeString("es-ES", { hour: "2-digit", minute: "2-digit" }),
    };
  }

  async function loadPortalRecords(employeeId) {
    setIsLoadingPortalRecords(true);
    try {
      const records = await supabaseRequest(
        `time_records?select=id,record_type,recorded_at,local_date&employee_id=eq.${encodeURIComponent(employeeId)}&order=recorded_at.desc&limit=10`
      );
      setPortalRecords(records.map(mapPortalRecord));
      return true;
    } catch (error) {
      setPortalRecords([]);
      setPortalMessage(`Error cargando tus fichajes: ${error.message}`);
      return false;
    } finally {
      setIsLoadingPortalRecords(false);
    }
  }

  function getPortalCurrentMonthRange() {
    const now = new Date();
  ...
  async function loadPortalMonthSummary(employeeId) {
    setIsLoadingPortalMonthSummary(true);
    try {
      const { start, end } = getPortalCurrentMonthRange();
      const records = await supabaseRequest(
        `time_records?select=id,record_type,recorded_at,local_date,note&employee_id=eq.${encodeURIComponent(employeeId)}&local_date=gte.${start}&local_date=lte.${end}&order=recorded_at.desc`
      );
      const daysWithRecords = new Set(records.map((record) => record.local_date).filter(Boolean)).size;
      const incidences = records
        .filter((record) => record.record_type === "ajuste" || record.note)
        .slice(0, 3)
        .map((record) => ({
          id: record.id,
          date: record.local_date || record.recorded_at?.slice(0, 10) || "",
          text: record.note || `Registro ${record.record_type}`,
        }));

      setPortalMonthSummary({
      ...
      setPortalMonthSummary(null);
      if (error.message.includes("local_date") || error.message.includes("schema cache")) {
        setPortalMessage("Error cargando resumen mensual: falta la columna local_date en time_records.");
      } else {
        setPortalMessage(`Error cargando resumen mensual: ${error.message}`);
      }
      return false;
    } finally {
      setIsLoadingPortalMonthSummary(false);
    }
  }

  function mapPortalDocument(document) {
    const uploadedAt = document.uploaded_at ? new Date(document.uploaded_at) : null;
    return {
      id: document.id,
      type: document.document_type || "Documento",
      period: document.period || "Sin periodo",
      fileName: document.original_filename || "Archivo sin nombre",
      filePath: document.file_path || "",
      uploadedAt: uploadedAt && !Number.isNaN(uploadedAt.getTime())
        ? uploadedAt.toLocaleDateString("es-ES", { dateStyle: "short" })
        : "Sin fecha",
    };
  }

  async function loadPortalDocuments(employeeId) {
    setIsLoadingPortalDocuments(true);
    try {
      const documents = await supabaseRequest(
        `employee_documents?select=id,document_type,period,original_filename,uploaded_at,file_path&employee_id=eq.${encodeURIComponent(employeeId)}&order=uploaded_at.desc`
      );
      setPortalDocuments(documents.map(mapPortalDocument));
      return true;
    } catch (error) {
      setPortalDocuments([]);
      setPortalMessage(`Error cargando tus nóminas: ${error.message}`);
      return false;
    } finally {
      setIsLoadingPortalDocuments(false);
    }
  }

  async function downloadPortalDocument(document) {
    if (!document.filePath) {
      setPortalMessage("Error descargando documento: falta file_path en employee_documents.");
      return;
    }

    try {
      setPortalMessage("Preparando descarga...");
      const cleanUrl = SUPABASE_URL.replace(/\/+$/, "").replace(/\/rest\/v1$/, "");
      const cleanPath = document.filePath.trim();
      const safePath = cleanPath
        .split("/")
        .map((part) => encodeURIComponent(part))
        .join("/");
      const requestUrl = `${cleanUrl}/storage/v1/object/sign/employee-documents/${safePath}`;
      console.log("[downloadPortalDocument] cleanPath", cleanPath);
      console.log("[downloadPortalDocument] safePath", safePath);
      console.log("[downloadPortalDocument] request URL", requestUrl);
      ...
      if (!response.ok) {
        console.log("[downloadPortalDocument] signed URL error", {
          status: response.status,
          responseText,
        });
        throw new Error(`Storage ${response.status}: ${responseText || response.statusText}`);
      }
      ...
      const downloadUrl = signedURL.startsWith("http")
        ...
        : signedURL.startsWith("/object/sign/")
          ? `${cleanUrl}/storage/v1${signedURL}`
          : `${cleanUrl}${signedURL}`;
      window.open(downloadUrl, "_blank", "noopener,noreferrer");
      setPortalMessage("Enlace temporal generado durante 60 segundos.");
    } catch (error) {
      setPortalMessage(`Error descargando documento: ${error.message}`);
    }
  }

  async function loginEmployeePortal() {
    setPortalMessage("Comprobando acceso...");
    ...
    try {
      ...
      const authenticatedEmail = data.user?.email || email.trim();
      setPassword("");
      let employeeRows;
      try {
        employeeRows = await supabaseRequest(
          `employees?select=id,full_name,email&email=eq.${encodeURIComponent(authenticatedEmail)}&limit=1`
        );
      } catch (error) {
        if (error.message.includes("email") || error.message.includes("schema cache")) {
          throw new Error("No existe la columna employees.email en Supabase. Añádela y vincula cada empleado con su email de Auth.");
        }
        throw error;
      }

      if (employeeRows.length === 0) {
        throw new Error("No hay ningún empleado vinculado a este email. Revisa employees.email en Supabase.");
      }

      const employee = employeeRows[0];
      const recordsLoaded = await loadPortalRecords(employee.id);
      const monthSummaryLoaded = await loadPortalMonthSummary(employee.id);
      const documentsLoaded = await loadPortalDocuments(employee.id);
      setEmployeeSession(data);
      setPortalEmployee({
        id: employee.id,
        name: employee.full_name || authenticatedEmail,
        email: employee.email || authenticatedEmail,
      });
      if (recordsLoaded && monthSummaryLoaded && documentsLoaded) {
        setPortalMessage("Sesión iniciada y empleado identificado.");
      }
    } catch (error) {
  ...
  async function logoutEmployeePortal() {
    const accessToken = employeeSession?.access_token;
    setPortalMessage("Cerrando sesión...");
    ...
    try {
      ...
      setPortalDocuments([]);
      setPassword("");
      setPortalMessage("Sesión cerrada.");
    }
  }

  if (employeeSession) {
    const userEmail = employeeSession.user?.email || email;
    const employeeName = portalEmployee?.name || "Empleado sin vincular";
        ...
        <Card className="rounded-3xl shadow-sm">
          <CardContent className="space-y-5 p-6">
            <div>
              ...
              <p className="text-lg font-semibold text-slate-900">{employeeName}</p>
              <p className="text-sm text-slate-500">{userEmail}</p>
            </div>

            <Button onClick={logoutEmployeePortal} variant="secondary" className="w-full rounded-2xl p-4 text-base">
              Cerrar sesión
            </Button>

            <div className="space-y-3">
              <div className="rounded-2xl border border-slate-200 bg-white p-4">
                <h3 className="text-sm font-bold text-slate-900">Resumen del mes</h3>
                {isLoadingPortalMonthSummary ? (
                  <p className="mt-3 text-sm text-slate-500">Cargando resumen...</p>
                ) : !portalMonthSummary ? (
                  <p className="mt-3 text-sm text-slate-500">No se pudo cargar el resumen mensual.</p>
                ) : (
                  <div className="mt-3 space-y-3">
                    <div className="grid grid-cols-2 gap-2">
                      <div className="rounded-xl bg-slate-50 p-3">
                        <p className="text-xs text-slate-500">Registros</p>
                        <p className="text-lg font-bold">{portalMonthSummary.totalRecords}</p>
                      </div>
                      <div className="rounded-xl bg-slate-50 p-3">
                        <p className="text-xs text-slate-500">Días</p>
                        <p className="text-lg font-bold">{portalMonthSummary.daysWithRecords}</p>
                      </div>
                    </div>

                    <div>
                      <p className="text-xs font-bold uppercase tracking-wide text-slate-500">Últimas incidencias</p>
                      {portalMonthSummary.incidences.length === 0 ? (
                        <p className="mt-2 text-sm text-slate-500">Sin incidencias este mes.</p>
                      ) : (
                        <div className="mt-2 space-y-2">
                          {portalMonthSummary.incidences.map((incidence) => (
                            <div key={incidence.id} className="rounded-xl bg-amber-50 px-3 py-2 text-xs text-amber-900">
                              <strong>{incidence.date}</strong> · {incidence.text}
                            </div>
                          ))}
                        </div>
                      )}
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-4">
                <h3 className="text-sm font-bold text-slate-900">Mis fichajes</h3>
                {isLoadingPortalRecords ? (
                  <p className="mt-3 text-sm text-slate-500">Cargando fichajes...</p>
                ) : portalRecords.length === 0 ? (
                  <p className="mt-3 text-sm text-slate-500">No hay fichajes registrados todavía.</p>
                ) : (
                  <div className="mt-3 space-y-2">
                    {portalRecords.map((record) => (
                      <div key={record.id} className="grid grid-cols-[1fr_auto] gap-3 rounded-xl bg-slate-50 px-3 py-2 text-sm">
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-4">
                <h3 className="text-sm font-bold text-slate-900">Mis nóminas</h3>
                {isLoadingPortalDocuments ? (
                  <p className="mt-3 text-sm text-slate-500">Cargando nóminas...</p>
                ) : portalDocuments.length === 0 ? (
                  <p className="mt-3 text-sm text-slate-500">No hay nóminas o documentos disponibles todavía.</p>
                ) : (
                  <div className="mt-3 space-y-2">
                    {portalDocuments.map((document) => (
                      <div key={document.id} className="rounded-xl bg-slate-50 px-3 py-2 text-sm">
                        <div className="flex items-start justify-between gap-3">
                          <span className="font-semibold text-slate-700">{document.type}</span>
                          <span className="text-xs text-slate-500">{document.uploadedAt}</span>
                        </div>
                        <p className="mt-1 text-xs font-medium text-slate-500">{document.period}</p>
                        <p className="mt-1 break-words text-xs text-slate-600">{document.fileName}</p>
                        <Button onClick={() => downloadPortalDocument(document)} variant="outline" className="mt-3 w-full rounded-xl px-3 py-2 text-sm">
                          Descargar
                        </Button>
                      </div>
                    ))}
              ...
              {lockedItems.map((item) => (
                <div key={item} className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-700">
                  {item}
                  <span className="mt-1 block text-xs font-medium text-slate-400">Bloqueado por ahora</span>
                </div>
              ))}
            </div>

            {portalMessage && (
              <div className="rounded-2xl bg-slate-50 p-3 text-xs font-medium text-slate-600">
                {portalMessage}
              </div>
            )}
          </CardContent>
      ...
      <Card className="rounded-3xl shadow-sm">
        <CardContent className="space-y-4 p-6">
          <div>
            ...
            <p className="text-sm text-slate-500">Accede a tu zona privada</p>
          </div>
          ...
          <div className="space-y-3">
            ...
            <Button onClick={loginEmployeePortal} className="w-full rounded-2xl p-4 text-base">
              Entrar
            </Button>
          </div>
...
function KioskPanel({
  ...
  return (
    <section className="mx-auto max-w-6xl">
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,2.4fr)_minmax(280px,1fr)]">
        <div>
          <Card className="rounded-3xl shadow-sm">
            <CardContent className="space-y-5 p-6">
              <div>
...
export default function AppFichajeEmpleados() {
  ...
  const [documentType, setDocumentType] = useState("nomina");
  const [documentPeriod, setDocumentPeriod] = useState(currentMonthISO());
  ...
  async function addRecord(type) {
    if (!selectedEmployee) {
      setMessage("Primero seleccióna un empleado.");
      return;
    }
  ...
  async function addAdjustment() {
    if (!selectedEmployee) {
      setMessage("Seleccióna un empleado.");
      return;
    }
  ...
  async function addEmployee() {
    ...
    try {
      ...
      if (!organizationId) throw new Error("No existe ningúna organización. Crea primero Letras a la Taza en organizations.");
  ...
  async function resetEmployeePin() {
    if (!resetPinEmployeeId) {
      setMessage("Seleccióna primero un empleado.");
      return;
    }
  ...
  async function uploadEmployeeDocument() {
    console.log("[uploadEmployeeDocument] inicio");
    const employeeId = documentEmployeeId;
    const cleanDocumentType = safeFileName(documentType);
    const cleanPeriod = safeFileName(documentPeriod);

    if (!employeeId || !cleanDocumentType || !cleanPeriod || !documentFile) {
      setMessage("Selecciona empleado, tipo, periodo y archivo antes de subir el documento.");
      return;
    }

    const originalFileName = documentFile.name || "documento";
    const extensionMatch = originalFileName.match(/\.([a-z0-9]+)$/i);
    const extension = extensionMatch ? extensionMatch[1].toLowerCase() : "bin";
    const timestamp = Date.now();
    const filePath = `${employeeId}/${cleanDocumentType}-${cleanPeriod}-${timestamp}.${extension}`;
    const cleanUrl = SUPABASE_URL.replace(/\/+$/, "").replace(/\/rest\/v1$/, "");
    console.log("[uploadEmployeeDocument] datos", {
      employeeId,
    ...
    setIsLoading(true);
    try {
      const storageResponse = await fetch(`${cleanUrl}/storage/v1/object/employee-documents/${filePath}`, {
        method: "POST",
        headers: {
          apikey: SUPABASE_ANON_KEY,
          Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
      ...
      if (!storageResponse.ok) {
        ...
        console.log("[uploadEmployeeDocument] storage error", {
          status: storageResponse.status,
          responseText: errorText,
        });
        throw new Error(`Storage ${storageResponse.status}: ${errorText || storageResponse.statusText}`);
      }
      const storageText = await storageResponse.text();
      console.log("[uploadEmployeeDocument] storage ok", {
        status: storageResponse.status,
        responseText: storageText,
      });

      const insertResponse = await fetch(`${cleanUrl}/rest/v1/employee_documents`, {
        method: "POST",
        headers: {
          apikey: SUPABASE_ANON_KEY,
          Authorization: `Bearer ${SUPABASE_ANON_KEY}`,
      ...
      console.log("[uploadEmployeeDocument] insert employee_documents", {
        status: insertResponse.status,
        responseText: insertText,
      });

      if (!insertResponse.ok) {
        throw new Error(`employee_documents ${insertResponse.status}: ${insertText || insertResponse.statusText}`);
      }

      setDocumentFile(null);
      if (documentFileInputRef.current) documentFileInputRef.current.value = "";
      setMessage(`Documento subido correctamente para ${getEmployeeDisplayName(employeeId, employees)}.`);
    } catch (error) {
      setMessage(`Error subiendo documento: ${error.message}`);
    } finally {
      setIsLoading(false);
    }
  }
  ...
  async function generateDailyReportPreview() {
    try {
      ...
      setMessage(`Informe díario generado. Código de verificación: ${report.signature.shortCode}.`);
    } catch (error) {
      setMessage(`Error generando informe díario: ${error.message}`);
    }
  }
  ...
  async function downloadDailyPDF() {
    try {
      ...
      setLastSignature(report.signature);
      setMessage(`PDF díario descargado. Código de verificación: ${report.signature.shortCode}.`);
    } catch (error) {
      setMessage(`Error descargando PDF díario: ${error.message}`);
    }
  }
  ...
  function printReportPreview() {
    ...
    setPrintMode(true);
    setMessage("Modo impresión activado. Si no aparece el díalogo, pulsa Ctrl+P y elige Guardar como PDF.");
  ...
  function selectRawHTML() {
    ...
    window.setTimeout(() => {
      if (reportTextAreaRef.current) {
        ...
        setMessage("HTML selecciónado. Pulsa Ctrl+C para copiarlo manualmente.");
      }
    }, 50);
  }
        ...
        <header className="flex flex-col gap-4 rounded-3xl bg-white p-6 shadow-sm md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-2 text-slate-600">
              {viewMode === "kiosk" ? "Modo kiosko para tablet de fichaje." : "Panel de administracion."}
            </p>
            <p className="mt-1 text-xs font-bold text-blue-700">Versión {APP_VERSION}</p>
          </div>
              ...
              <CardContent className="space-y-4 p-6">
                ...
                {lastSignature && (
                  <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">
                    Ultima firma generada: <strong>{lastSignature.shortCode}</strong>
                  </div>
                )}
              </CardContent>
            </Card>
            ...
            <Card className="rounded-3xl shadow-sm">
              <CardContent className="space-y-5 p-6">
                <div>
                  ...
                  <p className="text-sm text-slate-500">Guarda nominas y documentos privados del empleado.</p>
                </div>

                <div className="grid gap-3 md:grid-cols-[1.2fr_160px_140px_1.4fr_auto]">
                  <select
                    value={documentEmployeeId}
                    onChange={(event) => setDocumentEmployeeId(event.target.value)}
                    className="w-full rounded-2xl border border-slate-200 bg-white p-3 outline-none focus:ring-2 focus:ring-slate-300"
                  ...
                  >
                    ...
                    {employees.map((employee) => (
                      <option key={employee.id} value={employee.id}>
                        {employee.name}{employee.isActive === false ? " - baja" : ""}
                      </option>
                    ))}
                  </select>
                  ...
                  <input
                    ...
                    onChange={(event) => setDocumentPeriod(event.target.value)}
                    placeholder="2026-04"
                    className="w-full rounded-2xl border border-slate-200 bg-white p-3 outline-none focus:ring-2 focus:ring-slate-300"
                    disabled={isLoading}
                  />
                  ...
                  <input
                    ...
                    className="w-full rounded-2xl border border-slate-200 bg-white p-3 text-sm outline-none focus:ring-2 focus:ring-slate-300"
                    disabled={isLoading}
                  />

                  <Button onClick={uploadEmployeeDocument} className="rounded-2xl" disabled={isLoading}>
                    Subir
                  </Button>
                </div>
              </CardContent>
            ...
            <Card className="rounded-3xl shadow-sm">
              <CardContent className="space-y-5 p-6">
                <div>
                  ...
                  <p className="text-sm text-slate-500">Vista visual de la jornada del empleado selecciónado.</p>
                </div>

                {dayTimeline.length === 0 ? (
                  <div className="rounded-2xl border border-slate-200 bg-white p-6 text-center text-sm text-slate-500">
                    No hay registros para este empleado en la fecha selecciónada.
                  </div>
                ) : (
        ...
        {showReport && viewMode === "admin" && isAdminUnlocked && (
          <Card className="rounded-3xl shadow-sm">
            <CardContent className="space-y-4 p-6">
              ...
              {showRawHTML && (
                <div className="space-y-2">
                  <p className="text-sm text-slate-600">HTML selecciónable. Pulsa Ctrl+C para copiarlo y pégalo en un archivo .html o documento.</p>
                  <textarea
